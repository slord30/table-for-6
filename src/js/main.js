...
searchBtn.addEventListener("click", async () => {
  ...
  const checkFilters = Array.from(
    document.querySelectorAll(".recipe-filter:checked"),
      ...
      if (!recipes || recipes.length === 0) {
        ...
        recipeGrid.innerHTML = recipes
          .map((hit) => {
            ...
            return html.replace(
              'class="recipe-card',
              `data-full="${fullData}" class="recipe-card`,
            );
          })
          .join("");
      }
...
recipeGrid.addEventListener("click", async (e) => {
  ...
  if (e.target.classList.contains("nutrition-btn")) {
    ...
    const currentServings = parseInt(
      card.querySelector(".servings-input").value || 1,
    );
    const originalYield = parseInt(
      card.querySelector(".orig-yield").textContent || 1,
    );
    ...
    const rawIngredients = JSON.parse(
      decodeURIComponent(card.dataset.ingredients || "[]"),
    );
    const cleanIngredients = rawIngredients.map((ing) =>
...
recipeGrid.addEventListener("input", (e) => {
  if (e.target.classList.contains("servings-input")) {
    ...
    const orginalYield = parseInt(
      card.querySelector(".orig-yield").textContent,
    );
